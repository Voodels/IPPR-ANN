...
2nd and 3rd may 
3 hours performance 
2 chits -> 1 IPPR and 1 ANN 
Syllabus all in theory 

    IPPR

    To use OpenCV 
    use either RGB or GRAYSCALE 
    and small operations are expected 
        questions related to 
        Colour image 
        chaning the image domain 
        RGB-cmy and colour models question 
        Aspect ratio
            demonstrate impact how compression works 
        Image enhancement 
        Histogram Histo Equilization Img and its impact 
        Geo Transformation 1st quadrant 
        Translation rotation reflection Sheer Align Scaling 
        
        Write theory + code 
        
        Image transform in freq domain 
            Fourier Transform 
            Cosing 
            Wavelet when hh domain(segmentation) ll domain(multi resolution transform ) 
            and retrive back 

        Segmentation 
            Sovel Prevet And tani edge detection 
            Threasholding an
        Morphology 
            Dilation and erosing 
        Lowpass HighPass 
    ...
    Expected Output:
    a)Display of Input Image and three RGB split channel images
    b)Display of Input Image and three CMY split channel images
    -Demonstrate images with appropriate captions.
    Note: Image acquisition is preferred with your own mobile device capturing the scene
    from WCE campus.
    Expecting Individual and independent task creating unique image by every student.
            Assignment
                2
                ...
                // Color image à Gray
                image à Negative of Gray  //

    ANN
    1. Train a network with three different inputs and initial weight vector with Perceptron learning rule
using python.
2. Train a network with three different inputs and initial weight vector with Hebbian learning rule
using python.
3. Train a network with three different inputs and initial weight vector with Delta learning rule using
python. Assume λ = 1 and c=0.1.
4. Realization of logical AND, OR function using Perceptron learning rule.
5. Classify handwritten digits using Tensor flow.
6. Design a model for speech recognition.
7. Design an expert system for prediction of Parkinson disease.
OR
Stock Market Price prediction using feed forward algorithm.
...
/---------------------------/
# Comprehensive Study Guide for IPPR and ANN
## May 2nd and 3rd Performance (3 hours)

This guide covers key topics and assignments for both Image Processing and Pattern Recognition (IPPR) and Artificial Neural Networks (ANN) courses. Each section is organized to help you prepare effectively.
